  ...
  return (
    <footer className="flexCenter pb-24 pt-20 " >
      <div className="max_padd_container flex w-full flex-col gap-14">
        ...
        <Link to={"/"} className={"mb-10 bold-20"}>Shopper</Link>
        ...
        <p className="text-center  regular-14 teext-gray-30"> 2024 shoppe all rights reserved</p>
      </div>
    </footer>
  )
}
const FooterColumn=({title, children})=>{
  return(
    <div className="flex flex-col gap-5">
      <h4 className="bold-18 whitspace-nowrap">{title}</h4>
      {children}
    </div>
  )
}
